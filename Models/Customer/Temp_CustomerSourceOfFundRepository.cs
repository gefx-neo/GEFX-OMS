...
{
	public class Temp_CustomerSourceOfFundRepository : ITemp_CustomerSourceOfFundRepository
	{
		private DataAccess.GreatEastForex db;

		public Temp_CustomerSourceOfFundRepository()
		{
			db = new DataAccess.GreatEastForex();
		}

		public IQueryable<Temp_CustomerSourceOfFunds> Select()
		{
			var result = from c in db.Temp_CustomerSourceOfFunds select c;

			return result;
		}

		public Temp_CustomerSourceOfFunds GetSingle(int id)
		{
			try
			{
				IQueryable<Temp_CustomerSourceOfFunds> records = Select();
		...
		public Temp_CustomerSourceOfFunds GetSourceOfFund(int CustomerParticularId)
		{
			try
			{
				IQueryable<Temp_CustomerSourceOfFunds> records = Select();
		...
		public bool Add(CustomerSourceOfFund addData)
		{
			try
			{
				Temp_CustomerSourceOfFunds temp = new Temp_CustomerSourceOfFunds();
				...
				db.Temp_CustomerSourceOfFunds.Add(temp);
		...
		public bool Update(int id, CustomerSourceOfFund updateData)
		{
			try
			{
				Temp_CustomerSourceOfFunds data = db.Temp_CustomerSourceOfFunds.Where(e => e.ID == id).FirstOrDefault();
		...
		public bool Delete(int id)
		{
			try
			{
				Temp_CustomerSourceOfFunds data = db.Temp_CustomerSourceOfFunds.Find(id);

				db.Temp_CustomerSourceOfFunds.Remove(data);
